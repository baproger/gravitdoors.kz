#!/usr/bin/env bash
#
# Запасной путь: заливка кода на сервер rsync-ом, минуя GitHub.
#
# Основной путь — git: код в github.com/baproger/gravitdoors.kz, сервер
# клонирует его по deploy-ключу (deploy/bootstrap.sh) и обновляется через
# deploy.sh с git pull внутри. Этот скрипт нужен, когда GitHub недоступен
# или надо проверить незакоммиченную правку прямо на сервере. Он собирает
# фронтенд, копирует только нужное и запускает deploy.sh без git pull.
#
